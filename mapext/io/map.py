...
class cutoutgrid(Process):
    ...
    def run(self,src_lst,map_lst,globallims=False,circles=[12,16,20],srcScale=False):
        ...
        for src in src_lst:
            ...
            for m in map_lst:
                try:
                    map, unit, wcs = self.RUNDICT['OUTFILE'].rtrv_map(src,m.ID)
                except:
                    continue
                map[map==0] = np.nan
            ...
            try:
                ...
                if globallims==True:
                    lims = set_lims_inplace(maps.flatten(),max_step=1e5,lower=0.18)
                ...
                for idx in range(len(freq)):
                    ...
                    if freq[idx]<100:
                        t = ax.text(0.0,0.0,'{:1.2f} GHz'.format(freq[idx]),ha='left',va='bottom',transform=ax.transAxes,c='white')
                    else:
                        t = ax.text(0.0,0.0,'{:1.2f} THz'.format(freq[idx]/1e3),ha='left',va='bottom',transform=ax.transAxes,c='white')
                ...
                plt.savefig(self.RUNDICT['PLOTDIR']+src.NAME+'_CUTOUT_{}.pdf'.format(circles[0]))
            except:
                print('Fail')
